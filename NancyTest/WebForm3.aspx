<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="WebForm3.aspx.cs" Inherits="NancyTest.WebForm3" %>
			...
			$.ajax(
					{
						...
						url: '/handler/name',	/*Specifies the URL to send the request to. Default is the current page*/
			...
			$.ajax(
					{
						...
						error: errorCallbackAccount
					}
				);

			function successCallbackAccount(result, status, xhr) {
						$('#divMsg').populate(result);
			}

			function errorCallbackAccount(result, status, xhr) {
				$("#divMsg").css("display", "block");
				$('#divMsg').html("ERROR");
...
<body>
	<form id="form1">
	<div id="divDtl">
		<div style="display:none;background-color: Red; color: White; height: 40px; width: 100%" id="divMsg">
		</div>
